...
import com.kmstimes.nexon.api.dto.RankingApi;
 ...
 * <p>
 * {@link RankingApi} 인터페이스를 기반으로, 실제 API 요청을 수행하고,
 * 예외 처리를 일관되게 처리합니다.
 * <p>
...
public class RankingApiClient {
    ...
    /**
     * 생성자: Retrofit 인스턴스를 주입받아 {@link RankingApi} 를 초기화합니다.
     *
     * @param retrofit Retrofit 인스턴스
     */
    public RankingApiClient(Retrofit retrofit) {
        this.rankingApi = retrofit.create(RankingApi.class);
    }
     ...
     */
    public UnionRankingResponse getUnion(LocalDate date, String worldName, String ocid, int page) {
        WorldValidator.validate(worldName);
     ...
     */
    public GuildRankingResponse getGuild(LocalDate date, String worldName, int rankingType, String guildName, int page) {
        WorldValidator.validate(worldName);
     ...
     */
    public DojangRankingResponse getDojang(LocalDate date, String worldName, int difficulty, String characterClass, String ocid, int page) {
        WorldValidator.validate(worldName);
        ClassValidator.validate(characterClass);
     ...
     */
    public TheseedRankingResponse getTheseed(LocalDate date, String worldName, String ocid, int page) {
        WorldValidator.validate(worldName);
