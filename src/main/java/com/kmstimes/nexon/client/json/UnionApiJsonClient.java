...
import com.kmstimes.nexon.api.json.UnionApiJson;
 ...
 * <p>
 * {@link UnionApiJson} 인터페이스를 기반으로, 실제 API 요청을 수행하고,
 * 예외 처리를 일관되게 처리합니다.
 * <p>
 ...
 * </ul>
 */
public class UnionApiJsonClient {
    private final UnionApiJson unionApi;
    private final DateValidator dateValidator = new DateValidator(LocalDate.of(2023, 12, 21));

    /**
     * 생성자: Retrofit 인스턴스를 주입받아 {@link UnionApiJson} 를 초기화합니다.
     *
     * @param retrofit Retrofit 인스턴스
     */
    public UnionApiJsonClient(Retrofit retrofit) {
        this.unionApi = retrofit.create(UnionApiJson.class);
    }
     ...
     */
    public ResponseBody getUnion(String ocid, LocalDate date) {
     ...
     */
    public ResponseBody getUnionRaider(String ocid, LocalDate date) {
        OcidValidator.validate(ocid);
     ...
     */
    public ResponseBody getUnionArtifact(String ocid, LocalDate date) {
        OcidValidator.validate(ocid);
     ...
     */
    public ResponseBody getUnionChampion(String ocid, LocalDate date) {
        OcidValidator.validate(ocid);
