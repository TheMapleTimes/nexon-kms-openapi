...
import com.kmstimes.nexon.api.json.ProbabilityApiJson;
 ...
 * <p>
 * {@link ProbabilityApiJson} 인터페이스를 기반으로, 실제 API 요청을 수행하고,
 * 예외 처리를 일관되게 처리합니다.
 */
public class ProbabilityApiJsonClient {
    private final ProbabilityApiJson probabilityApi;
    ...
    /**
     * 생성자: Retrofit 인스턴스를 주입받아 {@link ProbabilityApiJson} 를 초기화합니다.
     *
     * @param retrofit Retrofit 인스턴스
     */
    public ProbabilityApiJsonClient(Retrofit retrofit) {
        this.probabilityApi = retrofit.create(ProbabilityApiJson.class);
    }
     ...
     */
    public ResponseBody getLegacyOuid(String userApiKey) {
     ...
     */
    public ResponseBody getOuid(String userApiKey) {
        ApiKeyValidator.validate(userApiKey);
     ...
     */
    public ResponseBody getStarforce(String userApiKey, int count, LocalDate date, String cursor) {
        ApiKeyValidator.validate(userApiKey);
     ...
     */
    public ResponseBody getPotential(String userApiKey, int count, LocalDate date, String cursor) {
        ApiKeyValidator.validate(userApiKey);
     ...
     */
    public ResponseBody getCube(String userApiKey, int count, LocalDate date, String cursor) {
        ApiKeyValidator.validate(userApiKey);
